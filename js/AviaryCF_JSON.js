...
function UploadFile(req,opts) {
	...
	Ext.Ajax.request({
		...
		success: 	function(req){
			var img = Ext.decode(req.responseText).response.response.files.file.url;
			originalimage = img;
			RefreshImage(img);
			Ext.WindowMgr.hideAll();
		}
...
function RenderOptions(req,opts) {
	...
	grid.dom.style.backgroundImage = "url("+renders.response.ostrichrenderresponse.url+")";
	grid.removeClass("x-hidden");
	renderoptions = renders.response.ostrichrenderresponse.renders.render;
	Ext.WindowMgr.hideAll();
}
/*****************************************************************************
